...
if not mongodb_url:
    ...
    try:
        ...
        if mongodb_database.startswith("mongodb://") or mongodb_database.startswith(
            "mongodb+srv://"
        ):
...
masked_url = (
    mongodb_url.replace(mongodb_url.split("@")[0].split("://")[1] + "@", "***:***@")
    if "@" in mongodb_url
    else mongodb_url
)
print(f"MongoDB connection configured: {masked_url}")
